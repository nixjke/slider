...
class SingleTip extends Tip implements ITip {
...
class IntervalTip extends Tip implements ITip {
  public paint({ target, value, pxValues, values }: ViewValues) {
    ...
    if (!tip) throw new Error('.sloder__tip - не был найден')

    tip.setAttribute('date-value', `${value}`)
...
export { Tip, IntervalTip, SingleTip, ITip }
